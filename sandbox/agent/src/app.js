...
app.get("/list-files", async (req, res) => {
    try {
        ...
        res.status(200).json({
            message: "Files list successfully",
            files: allFiles
        });
    } catch (err) {
        res.status(500).json({
            message: `Error listing directory: ${err.message}`,
            status: "error"
        });
    }
});
...
app.get("/read-files", async (req, res) => {
    ...
    if (!files) {
        return res.status(400).json({
            message: "Missing 'files' query parameter",
            status: "error"
        });
    }
    ...
    if (Array.isArray(files)) {
        ...
        return res.status(400).json({
            message: "Invalid 'files' parameter format",
            status: "error"
        });
    }
    ...
    const results = await Promise.all(fileList.map(async (file) => {
        ...
        try {
            ...
            return {
                [file.trim()]: `Error reading file: ${err.message}`
            };
        }
    }));
    
    res.status(200).json({
        message: "File contents checked",
        files: results
    });
});
